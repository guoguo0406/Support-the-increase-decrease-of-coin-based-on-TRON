...
import com.changyo.core.capsule.BytesCapsule;
...
public class DynamicPropertiesStore extends TronStoreWithRevoking<BytesCapsule> {
  ...
  private static class DynamicResourceProperties {
    ...
    private static final byte[] FREE_GAS_LIMIT = "FREE_GAS_LIMIT".getBytes();
  ...
  private static final byte[] CREATE_NEW_ACCOUNT_GAS_RATE = "CREATE_NEW_ACCOUNT_GAS_RATE"
      .getBytes();
  ...
  private static final byte[] TOTAL_SUPPLY = "TOTAL_SUPPLY".getBytes();

  private static final byte[] INCREASE_SUPPLY_ADDRESS = "INCREASE_SUPPLY_ADDRESS".getBytes();
  private static final byte[] DECREASE_SUPPLY_ADDRESS = "DECREASE_SUPPLY_ADDRESS".getBytes();
  ...
  private DynamicPropertiesStore(@Value("properties") String dbName) {
    ...
    try {
      this.getFreeGasLimit();
    } catch (IllegalArgumentException e) {
      this.saveFreeGasLimit(5000L);  // each account can get 5000 free gas per day
    }
    ...
    try {
      ...
      this.saveCreateAccountFee(0); // free for the first version
    }
    ...
    try {
      this.getCreateNewAccountGasRate();
    } catch (IllegalArgumentException e) {
      this.saveCreateNewAccountGasRate(1L); //changed by committee later
    }

    try {
      this.getTransactionFee();
    } catch (IllegalArgumentException e) {
      this.saveTransactionFee(10L); // 10 cent/byte
    }
    ...
    try {
      ...
      this.saveTotalSupply(Args.getInstance().getTotalSupply());
    }

    try {
      this.getIncreaseSupplyAddress();
    } catch (IllegalArgumentException e) {
      this.saveIncreaseSupplyAddress(Wallet.decodeFromBase58Check(Args.getInstance().getIncreaseSupplyAddress()));
    }

    try {
      this.getDecreaseSupplyAddress();
    } catch (IllegalArgumentException e) {
      this.saveDecreaseSupplyAddress(Wallet.decodeFromBase58Check(Args.getInstance().getDecreaseSupplyAddress()));
    }
  }
  ...
  public void saveFreeGasLimit(long freeGasLimit) {
    this.put(DynamicResourceProperties.FREE_GAS_LIMIT,
        new BytesCapsule(ByteArray.fromLong(freeGasLimit)));
  }

  public long getFreeGasLimit() {
    return Optional.ofNullable(getUnchecked(DynamicResourceProperties.FREE_GAS_LIMIT))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(
            () -> new IllegalArgumentException("not found FREE_GAS_LIMIT"));
  }
  ...
  public void saveCreateNewAccountGasRate(long rate) {
    this.put(CREATE_NEW_ACCOUNT_GAS_RATE,
        new BytesCapsule(ByteArray.fromLong(rate)));
  }

  public long getCreateNewAccountGasRate() {
    return Optional.ofNullable(getUnchecked(CREATE_NEW_ACCOUNT_GAS_RATE))
        .map(BytesCapsule::getData)
        .map(ByteArray::toLong)
        .orElseThrow(
            () -> new IllegalArgumentException("not found CREATE_NEW_ACCOUNT_GAS_RATE2"));
  }
  ...
  public void saveTotalSupply(long totalSupply) {
    this.put(TOTAL_SUPPLY,
            new BytesCapsule(ByteArray.fromLong(totalSupply)));
  }

  public long getTotalSupply() {
    return Optional.ofNullable(getUnchecked(TOTAL_SUPPLY))
            .map(BytesCapsule::getData)
            .map(ByteArray::toLong)
            .orElseThrow(
                    () -> new IllegalArgumentException("not found TOTAL_SUPPLY"));
  }

  public void saveIncreaseSupplyAddress(byte[] newIncreaseSupplyAddress) {
    this.put(INCREASE_SUPPLY_ADDRESS,
            new BytesCapsule(newIncreaseSupplyAddress));
  }

  public byte[] getIncreaseSupplyAddress() {
    return Optional.ofNullable(getUnchecked(INCREASE_SUPPLY_ADDRESS))
            .map(BytesCapsule::getData)
            .orElseThrow(
                    () -> new IllegalArgumentException("not found INCREASE_SUPPLY_ADDRESS"));
  }


public void saveDecreaseSupplyAddress(byte[] newDecreaseSupplyAddress) {
        this.put(DECREASE_SUPPLY_ADDRESS,
        new BytesCapsule(newDecreaseSupplyAddress));
  }

public byte[] getDecreaseSupplyAddress() {
        return Optional.ofNullable(getUnchecked(DECREASE_SUPPLY_ADDRESS))
        .map(BytesCapsule::getData)
        .orElseThrow(
        () -> new IllegalArgumentException("not found DECREASE_SUPPLY_ADDRESS"));
  }
}
